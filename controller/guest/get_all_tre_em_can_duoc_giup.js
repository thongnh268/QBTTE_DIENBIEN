...
exports.get_all_tre_em_can_duoc_giup = function (req, res) {
    var perPage = 10
    var page = req.params.page || 1
    document.find({status: false}).skip((perPage * page) - perPage).limit(perPage).sort({ 'createDate': -1 }).exec(function (err, data) {
        if (err) {
            res.json({
                result: 'failed',
                data: {},
            ...
            document.count({status: false}).exec(function (err, count) {
                if (err) {
                    res.json({
                        result: 'failed',
                        data: {},
                    ...
                    return res.render('./guest/ThongTinTreEm', {
                        ...
                        link: "/thong-tin-tre-em/tre-em-can-giup-do/page"
                    })
                }
            });
        }
